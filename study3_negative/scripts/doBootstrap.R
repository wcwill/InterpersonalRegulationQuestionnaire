...
doBootRegression <- function(dataset, formula, mixedEffects = FALSE, numberOfIterations = 5000, 
  ...
  if(mixedEffects) { 
    ...
    thisLMER <- (lmer(formula, data=dataset)) 
    ...
    for (j in 1:numberOfIterations) {
      ...
      #### OLD. Dont use. thisLMER <- summary(lmer(formula, data=bootstrappedDataset))
      thisLMER <- (lmer(formula, data=bootstrappedDataset))
...
doBootLogRegression <- function(dataset, formula, mixedEffects = FALSE, numberOfIterations = 5000, 
                             confidenceInterval=.95, na.rm=TRUE) {
  cat("If it throws a subscript out of bounds error, it could be because of singularities in the lm. To be fixed\n")
  results = list()
  if(mixedEffects) { 
    ...
    # use this for glmer
    thisLMER <- (glmer(formula, data=dataset, family=binomial(logit)))
    ...
    for (j in 1:numberOfIterations) {
      bootstrappedDataset <- dataset[sample(nrow(dataset), nrow(dataset), replace = TRUE, prob=NULL), ]
      # use this for glmer
      thisLMER <- (glmer(formula, data=bootstrappedDataset, family=binomial(logit)))
    ...
    # end of glmer branch
  } else { # no mixed effects, use simple glm (commented out R-squared bootstrapping for glm)
    # preallocate the final data vector
    #bootstrappedRSquaredVector <- numeric(numberOfIterations)
    numberOfCoefficients = nrow(summary(glm(formula, data=dataset, family=binomial(logit)))$coefficients)
    bootstrappedCoefficients = matrix(ncol = numberOfIterations, nrow = numberOfCoefficients)
    for (j in 1:numberOfIterations) {
      bootstrappedDataset <- dataset[sample(nrow(dataset), nrow(dataset), replace = TRUE, prob=NULL), ]
      thisLM = summary(glm(formula, data=bootstrappedDataset, family=binomial(logit)))
      #bootstrappedRSquaredVector[j] <- thisLM$r.square  
    ...
    results$coeff = matrix(nrow=numberOfCoefficients, ncol=3)
    thisLM = summary(glm(formula, data=dataset, family=binomial(logit)))
    rownames(results$coeff) <- rownames(thisLM$coefficients)
    colnames(results$coeff) <- c("Estimate", "CI.low", "CI.high")
    #results$Rsquare = c(quantile(bootstrappedRSquaredVector, .500, na.rm), 
    #                    quantile(bootstrappedRSquaredVector, .025, na.rm), 
  ...
  return(results)
} # end of doBootLogRegression function
